...
namespace IndignaFwk.Web.FrontOffice.MultiTenant
{
    public static class Ensure
    {
        ...
        public static class Argument
        {
            ...
            /// <summary>
            /// Ensures the given argument DateTime is in the future
            /// </summary>
            /// <param name="value">Value to check for future</param>
            /// <param name="paramName">Name of the parameter to a method</param>
            /// <exception cref="System.ArgumentException">Thrown if <paramref name="value"/> is in the past or present</exception>
            public static void InFuture(DateTime value, string paramName = "")
            {
                if (value <= DateTime.Now)
                {
                    if (paramName.IsNullOrEmpty())
                        throw new ArgumentException("value must be in the future, not the past or present");

                    throw new ArgumentException(paramName, paramName + "must be in the future, not the past or present");
                }
            }

            /// <summary>
            /// Ensures the given argument is a non-empty URL
            /// </summary>
            /// <param name="value">Value to check for URL pattern</param>
            /// <param name="paramName">Name of the parameter to a method</param>
            /// <exception cref="System.ArgumentException">Thrown when the <paramref name="value"/> is not a properly formatted URL</exception>
            public static void IsUrl(string value, string paramName = "")
            {
                if (!value.IsUrl())
                {
                    if (paramName.IsNullOrEmpty())
                        throw new ArgumentException(string.Format("value {0} is not a URL", value));

                    throw new ArgumentException(string.Format("value {0} is not a URL", value), paramName);
                }
            }
        }
    }
